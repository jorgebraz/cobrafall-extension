...
- A bar above search results to hide cards already in your cubes, or show only those.
- A `+` button on card hover that adds the printing you are looking at to any board of any
  enabled cube.

Matching is by card name, so any printing on Scryfall counts as already in the cube.
...
| Path | Role |
| --- | --- |
| `background/service-worker.js` | Message router, sync scheduling, add-to-cube |
